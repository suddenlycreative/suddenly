...
class MAE_Service_Carousel_Widget extends Widget_Base {
    ...
    public function get_name() {
        return 'mae-service-carousel';
    }

    // The get_title() method, which again, is a very simple one, you need to return the widget title that will be displayed as the widget label.
    public function get_title() {
        return __( 'Service Carousel', 'masterlayer' );
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default'     => [
                    [
                        'title'  => __( 'Service #1', 'masterlayer' ),
                    ],
                    [
                        'title'  => __( 'Service #2', 'masterlayer' ),
                    ],
                    [
                        'title'  => __( 'Service #3', 'masterlayer' ),
                    ],
                ],
                'title_field' => '{{{ title }}}',
            ]
        ...
        $this->add_control(
            ...
            [
                ...
                'type' => Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .master-service:hover .sep' => 'background-color: {{VALUE}};',
                ],
                'condition' => [ 'sep!' => '' ]
            ]
        );
    ...
    protected function render() {
        ...
        <div class="master-carousel-box <?php echo esc_attr($cls); ?>" <?php echo $data; ?>>
            ...
            foreach ( $services as $index => $item ) { 
                $html = $title = $content = $image_url = $icon = $url = $cls1 = "";
                ...
                if ($settings['url_type'] != 'none')
                    $url = $this->render_link( $item['url'], $settings['url_text']);
            
                // HTML render
                $cls1 = ' item-carousel ' . $settings['sep'];

                ?>
                <div class="master-service <?php echo esc_attr($cls1); ?>">
                    <div class="thumb">
    ...
    public function render_link( $url, $text ) {
        ...
        if ($link['url_type'] == 'link') {
            ...
            ob_start(); ?>
            <div class="url-wrap">
                <a class="master-button small <?php echo esc_attr($cls); ?>" href="<?php echo esc_url($url['url']); ?>" <?php echo esc_attr($url_attr); ?>>
